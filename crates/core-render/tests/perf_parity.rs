...
fn trimmed_diff_saves_columns() {
    ...
    // Replace AAAAAA with B (saves >=5 columns => above threshold 4)
    {
        let buf = model.state_mut().active_buffer_mut();
        let mut pos = Position::new(0, 6); // after 'prefix'
        for _ in 0..6 {
...
fn mixed_workload_counters_consistency() {
    ...
    let mut eng = RenderEngine::new();
    let layout = Layout::single(W, H);
    eng.render_full(model.state(), &model.active_view().clone(), &layout, W, H)
        .unwrap();
    ...
    for target in [1usize, 2] {
        ...
        eng.render_cursor_only(model.state(), &v, &layout, W, H)
            .unwrap();
    }
    ...
    // 3. Interior trim edit on current top line to trigger trimmed diff.
    {
        ...
        eng.render_lines_partial(model.state(), &v_after, &layout, W, H, &mut dirty)
            .unwrap();
    }
