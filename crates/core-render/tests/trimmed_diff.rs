...
fn trim_middle_interior_success() {
    ...
    eng.render_full(model.state(), &view0, &layout, 80, 8)
        .unwrap();
    // Edit first line interior: replace "bravo" with "B" preserving large prefix+suffix so trimming should trigger.
    {
        let buf = model.state_mut().active_buffer_mut();
        // Find byte offset of 'b' in first line: "alpha bravo charlie" => 'b' after "alpha ": 6
        let mut pos = Position::new(0, 6); // start of bravo
        // delete 'bravo' (5 chars) by grapheme
        for _ in 0..5 {
    ...
    eng.render_lines_partial(model.state(), &view_after, &layout, 80, 8, &mut dirty)
        .unwrap();
...
fn trim_unicode_wide_cluster() {
    ...
    eng.render_full(model.state(), &view0, &layout, 80, 8)
        .unwrap();
    // Replace middle emoji cluster with single ASCII to create large savings
    {
        let buf = model.state_mut().active_buffer_mut();
        // After first emoji boundary
        let mut pos = Position::new(0, "😀".len());
        // delete second emoji cluster
        buf.delete_grapheme_at(&mut pos);
        buf.insert_grapheme(&mut pos, "X");
    }
    let mut dirty = DirtyLinesTracker::new();
...
fn trim_line_shrink_behavior() {
    ...
    let mut eng = RenderEngine::new();
    eng.render_full(model.state(), &model.active_view().clone(), &layout, 80, 8)
        .unwrap();
    {
        let buf = model.state_mut().active_buffer_mut();
...
fn scroll_shift_preserves_prev_text_for_trim() {
    ...
    eng.render_scroll_shift(model.state(), &view_scrolled, &layout, 80, 8, 0, 1)
        .unwrap();
...
fn cache_shift_invariants() {
    ...
    let view_after = model.active_view().clone();
    eng.render_scroll_shift(model.state(), &view_after, &layout, 80, 8, 0, 2)
        .unwrap();
...
fn resize_invalidation_repopulates_prev_text() {
    ...
    eng.render_full(model.state(), &model.active_view().clone(), &layout, 100, 8)
        .unwrap();
    assert!(eng.test_prev_text(0).is_some());
}
