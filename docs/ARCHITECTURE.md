...
3. Input thread reads terminal events and sends Input events.
4. EventDrivenEditor processes events, mutates Editor as needed, and sends UI RedrawRequest when state changes.
5. Editor::render() snapshots EditorRenderState and asks UI to draw via Terminal.
