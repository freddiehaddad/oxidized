...
- [x] Step 10 – Scroll region enable + shift path (complete) – introduced `SCROLL_SHIFT_MAX` threshold (12) gating small vertical scrolls to an effective `Scroll` path. Added `render_scroll_shift` in `RenderEngine` performing cache line hash shift + selective entering line repaints, cursor & status overlay, and metrics (`scroll_region_shifts`, `scroll_region_lines_saved`). Scheduler now sets `effective=Scroll` for deltas within threshold; larger scrolls fallback to full. Tests cover scheduler effective decision (small vs large), downward and upward scroll-shift metrics, and repaint line count equals shift magnitude. Fallback safety: cold cache or width mismatch escalates to full.
