...
- [x] Step 13 – Status line cache skip unchanged (complete) – Added `prev_status` cache in `RenderEngine` and `status_skipped` metric. Full renders now compute the status string via `build_status` and store it directly (avoids reconstructing from padded Frame cells). Partial paths (cursor-only, lines, scroll shift) build a prospective status string; if identical to `prev_status`, they skip emission and increment `status_skipped`, otherwise repaint and refresh the cache. Guarantees correctness by always updating cache on repaint; safe no-op optimization isolated from other partial heuristics. Tests: (a) unchanged cursor-only frame increments `status_skipped`, (b) dirty flag mutation forces repaint and does not increment. Future status feature additions can force repaint simply by clearing or altering `prev_status`.
- [x] Step 14 – Ticker Event::Tick introduction (complete) – Added `Event::Tick` variant plus a 250ms tokio interval task emitting ticks through the bounded channel (maintains backpressure semantics). Ephemeral status expiry moved from per-loop unconditional check to Tick handler, preserving event-driven idle sleep (no busy wait). Tick processing expires messages and schedules `RenderDelta::StatusLine` only when content changes. Test `tick_event_expires_ephemeral_and_schedules_status` validates scheduling. Lays groundwork for future periodic tasks (metrics overlay refresh, cursor blink) without introducing polling.
- [x] Step 15 – Real :metrics snapshot output (complete) – Replaced placeholder ephemeral with structured multi-line snapshot capturing operator/register counters plus last render path & semantic scheduler metrics. Snapshots stored in `EditorState` (`last_render_path`, `last_render_delta`) immediately after successful render & scheduler consume without giving command layer direct access to renderer internals (maintains modularity). Output truncates to 10 lines (breadth-first safety) and uses a 5s TTL for readability. Added structured `tracing::info!(target="metrics", ...)` log on each `:metrics` invocation emitting all counters (provides full visibility pending a future multi-line overlay UI). Updated existing metrics command test to assert presence of headings & numeric counters.
- [x] Step 16 – Repaint path refactor (full/partial/trimmed unify) (complete) – Introduced unified helpers inside `RenderEngine` to eliminate duplicated ANSI emission logic across partial repaint paths: (a) `paint_content_trim` for grapheme‑aware line emission (cursor-only, lines, scroll shift), (b) `overlay_cursor_cluster` for reverse‑video cursor styling with wide cluster handling, (c) `maybe_repaint_status` centralizing status skip logic + metric increment. Refactored `render_cursor_only`, `render_lines_partial`, and `render_scroll_shift` to use these helpers; trimmed diff fallback now calls the shared painter ensuring parity with full line repaints. Scroll shift path updated to reuse the same painter for entering lines and old cursor line repaint. Added Clippy suppression (`too_many_arguments`) only for the cursor overlay helper (transitional; future struct param may reduce arity). All existing tests (operators, partial parity, scroll shift invariants, trimmed diff scenarios, status skip) remained green (`cargo test` post-refactor) confirming no behavioral regression while reducing maintenance surface and aligning with upcoming performance parity tests in Step 17.
- [x] Step 17 – Integration perf parity tests (complete) – Added `perf_parity.rs` exercising: (a) cursor burst (multiple cursor-only partials, no new full frames, bounded repainted line delta), (b) trimmed diff savings (interior mutation triggers `trim_attempts`, `trim_success`, and positive `cols_saved_total` with limited repaint), (c) scroll shift sequence (repeated small shifts accumulate `scroll_region_shifts` & `scroll_region_lines_saved` without extra full frames), (d) mixed workload (cursor moves + scroll shift + trimmed edit) asserting counters advance coherently (no escalation, stable full frame count). Avoids wall-clock timing for determinism; leverages structural metrics introduced in prior steps. Captures performance parity guarantees foundational for Phase 4 closure.
- [x] Step 18 – Documentation updates (operators + perf paths) (complete) – Expanded crate docs: `core-actions` now reflects active count/operator paths & span resolver integration; `core-state` documents live register population + operator metrics exposure through `:metrics`; `core-render` module docs updated for scroll region shifts, trimmed diff heuristic, status skip cache, and unified helper refactor. README refreshed to enumerate Phase 4 runtime optimizations (scroll shifts, trimming, status skip) and real `:metrics` snapshot availability. No code behavior changes; tests unchanged.
- [x] Step 19 – Quality gate & Phase 4 closure (complete) – Executed full workspace test suite (all green); scanned codebase for residual critical TODO/FIXME markers (only non-blocking test extension TODO found). No unresolved partial rendering correctness gaps identified. Phase 4 exit criteria satisfied: operators (d/y/c) with counts, register rotation + metrics, scroll-region shift optimization, trimmed diff heuristic, status skip cache, real :metrics snapshot, performance parity tests, and documentation alignment. Ready to open Phase 5 planning (pastes, visual mode, multi-view, syntax pipeline). Breadth-first principle maintained: editor runnable at each commit with safe fallbacks for every optimization.
